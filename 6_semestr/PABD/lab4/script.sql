...
CREATE TABLE IF NOT EXISTS Uch_Plan (id_uch SERIAL, name_uch VARCHAR(50) UNIQUE NOT NULL, Speciality_id_spec INTEGER, PRIMARY KEY (id_uch),  FOREIGN KEY (Speciality_id_spec) REFERENCES Speciality (id_spec) ON DELETE RESTRICT ON UPDATE RESTRICT);
...
CREATE TABLE IF NOT EXISTS Gupr (duration INTEGER, Gupr_elem_id_gupr_elem INTEGER,  FOREIGN KEY (Gupr_elem_id_gupr_elem) REFERENCES Gupr_elem (id_gupr_elem) ON DELETE RESTRICT ON UPDATE RESTRICT);
